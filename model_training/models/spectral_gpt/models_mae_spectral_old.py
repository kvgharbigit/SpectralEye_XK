...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(
        ...
        self.voxels_per_mini_patch = patch_size ** 2 * self.t_pred_patch_size * in_chans

        self.decoder_norm = norm_layer(decoder_embed_dim)
        self.decoder_pred = nn.Linear(decoder_embed_dim, self.voxels_per_mini_patch, bias=True)
    ...
    def forward_loss(self, imgs, pred, mask):
        ...
        _imgs = torch.index_select(imgs,2,
                                   torch.linspace(0, imgs.shape[2] - 1, self.pred_t_dim).long().to(imgs.device))
        ...
        target_whole = target1.reshape(N, t, h * w, self.voxels_per_mini_patch)  # 2,4,256,192
        target_spatial = target_whole.sum(dim=1)  # 2,4,192
        pred_whole = pred.reshape(N, t, h * w, self.voxels_per_mini_patch)
...
if __name__ == '__main__':
    ...
    output = model(input)
    patch = model.module.patchify(input.unsqueeze(1))
    input_r = model.module.unpatchify(patch)

    print(torch.allclose(input, input_r.squeeze(1)))
